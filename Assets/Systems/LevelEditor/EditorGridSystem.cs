			...
			switch (child.Name)
			{
				...
				case "document":
					try
					{
						int posCol = int.Parse(child.Attributes.GetNamedItem("posX").Value);
						int posLig = int.Parse(child.Attributes.GetNamedItem("posY").Value);
						setTile(posLig, posCol, Cell.Document);
					}
					catch
					{
						Debug.Log("Warning: Skipped document from file " + levelKey + ". Wrong position!");
					}
					break;
				case "console":
					try
			...
			if (!paintableGrid.floorObjects.ContainsKey(tuplePos) || paintableGrid.floorObjects[tuplePos].type != cell) 
			{
				paintableGrid.floorObjects[tuplePos] =
					cell switch
					{
						...
						Cell.Document => new FloorObject(Cell.Document, Direction.Dir.North, line, col, false, false),
