...
public class LevelGenerator : FSystem {
		...
		foreach (XmlNode child in root.ChildNodes)
		{
			switch (child.Name)
			{
				...
				case "coin":
					...
					break;
				case "document":
					createDoc(int.Parse(child.Attributes.GetNamedItem("posX").Value), int.Parse(child.Attributes.GetNamedItem("posY").Value));
					break;
				case "console":
	...
	private void createDoc(int gridX, int gridY){
		gameData.totalDocToCollect ++;
		GameObject document = GameObject.Instantiate<GameObject>(Resources.Load ("Prefabs/Document") as GameObject, LevelGO.transform.position + new Vector3(gridY*3,1.69f,gridX*3), Quaternion.Euler(90,0,0), LevelGO.transform);
		document.GetComponent<Position>().x = gridX;
		document.GetComponent<Position>().y = gridY;
		GameObjectManager.bind(document);
	}
